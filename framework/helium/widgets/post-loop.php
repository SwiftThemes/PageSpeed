 ...
 */
class Helium_Widgets_PostLoop extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			__( 'Helium Post Loop', 'siteorigin-panels' ),
			array(
	...
	function widget( $args, $instance ) {
		...
		if ( strpos( '/' . $instance['template'], '/content' ) !== false ) {
			$index = 0;
			$width = isset($instance['width'])?absint($instance['width']):1200;
		    while ( have_posts() ) {
				the_post();
				$index++;
	...
	function form( $instance ) {
		...
		if ( class_exists( 'SiteOrigin_Widget' ) && class_exists( 'SiteOrigin_Widget_Field_Posts' ) ) {
            ...
            <p>
                ...
                <input type="checkbox" class="widefat" id="<?php echo $this->get_field_id( 'more' ) ?>"
                       name="<?php echo $this->get_field_name( 'more' ) ?>" <?php checked( $instance['more'] ) ?> /><br/>
            ...
            <p>
                ...
                <input type="text" class="small-text" id="<?php echo $this->get_field_id( 'posts_per_page' ) ?>"
            ...
            <p>
                ...
                <select id="<?php echo $this->get_field_id( 'sticky' ) ?>"
            ...
            <p>
                ...
                <input type="text" class="widefat" id="<?php echo $this->get_field_id( 'additional' ) ?>"
                       name="<?php echo $this->get_field_name( 'additional' ) ?>"
                       value="<?php echo esc_attr( $instance['additional'] ) ?>"/>
                <small>
					<?php
					echo preg_replace(
						'/1\{ *(.*?) *\}/',
	...
	private function get_helper_widget( $templates ) {
		if ( empty( $this->helper ) &&
			...
			$this->helper = new Helium_Widgets_PostLoop_Helper( $templates );
		}
...
if ( class_exists( 'SiteOrigin_Widget' ) ) {
	...
	class Helium_Widgets_PostLoop_Helper extends SiteOrigin_Widget {
		/**
		...
		function __construct( $templates ) {
			...
			parent::__construct(
				...
				array(
					...
					'width' => array(
						'type' => 'number',
						'label' => __( 'Width', 'siteorigin-panels' ),
						'description' => __( 'Width of the column this template will be used in', 'siteorigin-panels' ),
						'default' => 1200,
					),
					'more' => array(
						'type' => 'checkbox',
...
function myplugin_register_widgets() {
	register_widget( 'Helium_Widgets_PostLoop' );
}
