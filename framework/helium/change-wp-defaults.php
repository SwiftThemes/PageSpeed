...
if ( ! function_exists( 'he_move_comment_field_to_bottom' ) ) {
	function he_move_comment_field_to_bottom( $fields ) {
		...
		$fields['email']  = '<p class="comment-form-email"><label for="email">' . __( 'Email','page-speed') . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
		                    '<input id="email" name="email" placeholder="' . __( 'Email address', 'page-speed') . ( $req ? ' *' : '' ) . '"' . ( 1 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" maxlength="100" aria-describedby="email-notes"' . $aria_req . $html_req . ' /></p>';
		$fields['url'] = '<p class="comment-form-url"><label for="url">' . __( 'Website','page-speed') . '</label> ' .
		                 '<input id="url"  placeholder="'.__('Your website url','page-speed').'" name="url" type="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" maxlength="200" /></p>';
