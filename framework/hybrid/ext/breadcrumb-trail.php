...
class Breadcrumb_Trail {
	...
	protected function add_network_home_link() {
		...
		if ( is_multisite() && ! is_main_site() && true === $this->args['network'] ) {
			$this->items[] = sprintf( '<a href="%s" itemprop="item"  rel="home">%s</a>', esc_url( network_home_url() ), $this->labels['home'] );
		}
	}
	...
	protected function add_site_home_link() {
		...
		$this->items[] = sprintf( '<a href="%s" itemprop="item" %s>%s</a>', esc_url( home_url() ), $rel, $label );
	}
	...
	protected function add_blog_items() {
		...
		if ( is_paged() ) {
			$this->items[] = sprintf( '<a href="%s" itemprop="item" >%s</a>', esc_url( get_permalink( $post_id ) ), $title );
	...
	protected function add_singular_items() {
		...
		if ( $post_title = single_post_title( '', false ) ) {
			...
			if ( 1 || 1 < get_query_var( 'page' ) || is_paged() ) {
	...
	protected function add_term_archive_items() {
		...
		// @todo review the bypass.
		if ( 1 || is_paged() ) {
			$this->items[] = sprintf( '<a href="%s" itemprop="item" >%s</a>', esc_url( get_term_link( $term, $term->taxonomy ) ), single_term_title( '', false ) );
	...
	protected function add_search_items() {
		...
		if ( is_paged() ) {
			$this->items[] = sprintf( '<a href="%s" itemprop="item" >%s</a>', esc_url( get_search_link() ), sprintf( $this->labels['search'], get_search_query() ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s" itemprop="item" >%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'page-speed' ) ) ) );
				}

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s" itemprop="item" >%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'page-speed' ) ) ) );
				}

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s" itemprop="item" >%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'page-speed' ) ) ) );
				}
